...
-- 02_seed.sql (SQLite)
INSERT OR IGNORE INTO channel(name, type) VALUES
  ('Google Ads', 'search'),
  ('Meta Ads', 'social'),
...
INSERT OR IGNORE INTO campaign_channel_xref(campaign_id, channel_id)
SELECT c.campaign_id, ch.channel_id
FROM campaign c JOIN channel ch
WHERE (c.name='Holiday 2025 Awareness' AND ch.name IN ('Google Ads','Meta Ads'))
